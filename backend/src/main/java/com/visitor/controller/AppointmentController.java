...
public class AppointmentController {
    ...
    public Result<Map<String, Integer>> create(@RequestBody Appointment a, HttpServletRequest req) {
        ...
        if (a.getEndTime() != null) {
            appointmentService.checkTimeConflict(a.getHostId(), a.getStartTime(), a.getEndTime());
        }
        appointmentService.save(a);
        // 异步触发 AI 风险预警（RAG 检索 + 大模型评估）
        appointmentService.assessRiskIfNeeded(a.getId());
    ...
    public Result<Void> cancel(@PathVariable Integer id, HttpServletRequest req) {
        Integer userId = (Integer) req.getAttribute("userId");
    ...
    public Result<Void> restore(@PathVariable Integer id, HttpServletRequest req) {
        Integer userId = (Integer) req.getAttribute("userId");
        Appointment a = appointmentService.getById(id);
        if (a == null) return Result.error("预约不存在");
        // 数据级权限校验：只能恢复自己的预约（游客预约 visitorId 为 null，忽略此检查）
        if (a.getVisitorId() != null && !userId.equals(a.getVisitorId())) {
            return Result.error(403, "无权操作他人预约");
        }
        if (!AppointmentStatus.CANCELLED.equals(a.getStatus())) {
    ...
    public Result<Void> approve(@PathVariable Integer id, @RequestBody Map<String, String> body, HttpServletRequest req) {
        ...
        Integer operatorId = (Integer) req.getAttribute("userId");
        String role = (String) req.getAttribute("role");

        // 审批 + 发通知（同事务）
        appointmentService.approve(id, status, remark, operatorId, role);

        // 审批通过时异步生成 AI 话术（事务外，失败不影响审批）
        if (AppointmentStatus.APPROVED.equals(status)) {
            appointmentService.generateGreetingIfNeeded(id);
        }
    ...
    public Result<Map<String, Integer>> helper(@RequestBody Appointment a, HttpServletRequest req) {
        ...
        a.setStatus(AppointmentStatus.PENDING);
        if (a.getEndTime() != null) {
            appointmentService.checkTimeConflict(hostId, a.getStartTime(), a.getEndTime());
        }
        appointmentService.save(a);
        // 异步触发 AI 风险预警
        appointmentService.assessRiskIfNeeded(a.getId());
