...
public class GuardController {
    ...
    public Result<Map<String, Object>> verifyByName(@RequestBody Map<String, String> body) {
        ...
        List<Appointment> list = appointmentService.lambdaQuery()
                ...
                .in(Appointment::getStatus, AppointmentStatus.VERIFIABLE)
    ...
    private void sendGreetingNotification(Appointment a) {
        ...
        if (greeting != null && "completed".equals(greeting.getStatus())) {
            content = String.format(
                "{\"greetingText\":\"%s\",\"seatSuggestion\":\"%s\",\"notes\":\"%s\"}",
                escJson(greeting.getGreetingText()),
                escJson(greeting.getSeatSuggestion()),
                escJson(greeting.getNotes()));
        } else {
            content = String.format(
                "{\"greetingText\":\"欢迎%s莅临我司，%s将接待您。\",\"seatSuggestion\":\"\",\"notes\":\"\"}",
