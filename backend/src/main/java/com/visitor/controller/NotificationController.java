...
public class NotificationController {
    ...
    private void createUserNotificationsForAnnouncement(Notification n, Integer currentUserId) {
        ...
        for (User user : targetUsers) {
            ...
            else log.warn("为用户[{}]创建通知失败！可能 appointment_id NOT NULL 约束未解除", user.getId());
        }
        ...
        if (currentUserId != null) {
            ...
            if (saved) successCount++;
            else log.warn("为管理员[{}]创建通知失败！可能 appointment_id NOT NULL 约束未解除", currentUserId);
        }
