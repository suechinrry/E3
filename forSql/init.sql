...
--    密码建议BCrypt加密；存量明文登录时兼容处理
-- -----------------------------------------------------------
DROP TABLE IF EXISTS sys_user;
CREATE TABLE sys_user (
  id            INT AUTO_INCREMENT PRIMARY KEY,
  username      VARCHAR(50)  NOT NULL UNIQUE COMMENT '登录名',
  password      VARCHAR(100) NOT NULL COMMENT '密码（BCrypt加密，明文以$2a$开头的为密文）',
...
CREATE TABLE app_appointment (
  ...
  INDEX idx_host_time (host_id, start_time)
...
-- -----------------------------------------------------------
-- 10. 用户通知表（弹窗提醒专用，appointment_id 允许 NULL）
-- -----------------------------------------------------------
DROP TABLE IF EXISTS sys_user_notification;
CREATE TABLE sys_user_notification (
  id              INT AUTO_INCREMENT PRIMARY KEY,
  user_id         INT          NOT NULL COMMENT '接收通知的用户ID',
  type            ENUM('approved','rejected','greeting','announcement','risk_warning') NOT NULL COMMENT '通知类型：审批通过/审批拒绝/欢迎莅临/公告/风险预警',
...
-- -----------------------------------------------------------
-- 11. AI 风险预警记录表
--     新预约提交后，后端 RAG 检索相似被拒记录 → 大模型评估风险等级
-- -----------------------------------------------------------
DROP TABLE IF EXISTS app_risk_assessment;
CREATE TABLE app_risk_assessment (
  id              INT AUTO_INCREMENT PRIMARY KEY,
  appointment_id  INT          NOT NULL COMMENT '预约ID',
  risk_level      ENUM('low','medium','high') NOT NULL COMMENT '风险等级',
  risk_score      INT          NOT NULL DEFAULT 0 COMMENT '风险分数 0-100',
  reason          TEXT         NULL     COMMENT '预警理由',
  similar_cases   TEXT         NULL     COMMENT '相似历史案例(JSON)',
  create_time     DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP,
  INDEX idx_appointment (appointment_id),
  INDEX idx_level (risk_level)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='AI风险预警记录表';
